    ...
    protected function createFormConfig() {
        return parent::createFormConfig()
            ...
            ->addValidatorsForCreate([
                'email' => 'required|email|min:4|max:100|unique:' . AdminTableStructure::getTableName() . ',email',
                //'login' => 'required|regex:%^[a-zA-Z0-9@_.-]+$%is|min:4|max:100|unique:' . AdminTableStructure::getTableName() . ',login',
                'password' => 'required|min:6',
            ])
            ->addValidatorsForEdit([
                'id' => FormConfig::VALIDATOR_FOR_ID,
                'email' => 'required|email|min:4|max:100|unique:' . AdminTableStructure::getTableName() . ',email,{{id}},id',
                //'login' => 'required|regex:%^[a-zA-Z0-9@_.-]+$%is|min:4|max:100|unique:' . AdminTableStructure::getTableName() . ',login,{{id}},id',
                'password' => 'min:6',
            ])
            ;
    }
