    ...
    public function fire() {
        ...
        if ($writeAdminDbClasses) {
            ...
            File::load($subfolder->pwd() . 'Admin.php', true, 0755, 0644)
                ->write(\View::file($viewsPath . 'db/admin_object.php', $dataForViews)->render());
            File::load($subfolder->pwd() . 'AdminsTable.php', true, 0755, 0644)
                ->write(\View::file($viewsPath . 'db/admin_model.php', $dataForViews)->render());
            File::load($subfolder->pwd() . 'AdminsTableStructure.php', true, 0755, 0644)
                ->write(\View::file($viewsPath . 'db/admin_table_config.php', $dataForViews)->render());
            File::load($subfolder->pwd() . 'AdminsScaffoldConfig.php', true, 0755, 0644)
                ->write(\View::file($viewsPath . 'db/admin_scaffold_config.php', $dataForViews)->render());
        }
        // migration
        ...
        if ($writeMigration) {
            File::load($migrationFilePath, true, 0755, 0644)
                ->write(\View::file($viewsPath . 'db/admins_migration.php', $dataForViews));
        }
        $this->line('Done');
    }
}