...
(function (window) {
    ...
    var
            ...
            , $ = window.jQuery || window.Zepto || window.ender || window.$ || function (el) {
                        return el;
                    }
            ...
            , _now = performance.now ? function () {
                return performance.now();
            } : function () {
                return (new Date).getTime();
            }
            , _timeStart = function (ctx, label) {
                ctx._profilerTimers.push([label, _now(), 0]);
            }
            , _timeEnd = function (ctx, label) {
                ctx._profilerTimers.push([label, _now(), 1]);
            }
            ;
    ...
    var MyEmitter = klass(window.Emitter && Emitter.fn || {
                ...
                trigger: function (name, args) {
                    var
                            method = ('on-' + name).replace(/-(.)/g, function (a, b) {
    ...
    var Router = MyEmitter.extend(/** @lends Pilot.prototype */{
        ...
        __lego: function __(options) {
            ...
            options.el && $() && $(options.el).on('click tap', options.selector, {app: this}, function (evt) {
                ...
                if (el.target !== '_blank' && el.target !== '_self') {
                    ...
                    if (url === 'back') {
                        var defaultUrl = el.getAttribute('data-default-url') || el.getAttribute('data-fallback-url');
                        app.back(defaultUrl);
                    } else if (url === 'forward' || url === 'reload') {
                        app[url]();
                    } else {
                        var force = false;
                        if (!!options.reloadable) {
                            force = !el.hasAttribute('data-reloadable') && el.getAttribute('data-reloadable') !== '0';
                        } else {
                            force = el.getAttribute('data-reloadable') === '1';
                        }
                        app.nav(Router.getLocation(url), force);
                    }
                    app.trigger('userAction:navigate', [url, el]);
                }
            });
        ...
        _loadUnits: function (req/**Pilot.Request*/, originUnits) {
            ...
            while (task = units.shift()) {
                if (task && task.isActive(originUnits)) {
                    ...
                    if (typeof task.loadTemplate === 'function') {
                        queue.push(task.loadTemplate(req).then(_bind(task, task.setTemplate)));
                    }
        ...
        addToHistory: function (url) {
            ...
            this.historyIdx = this.history.push(url) - 1;
        },
        ...
        _nav: function (url, byHistory, force) {
            ...
            if (force || !this.activeRequest || this.activeRequest.url != req.url) {
                ...
                if (units.length) {
                    req.routeDetected = true;
                    var isReload = force && this.activeRequest.url === req.url;
                    for (var i in units) {
                        if (typeof units[i].beforeRouteLoad === 'function') {
                            units[i].beforeRouteLoad(req, isReload);
                        }
                    }
                    ...
                    ;
                    this.trigger('route:found', req);
                    df.done(function () {
                        for (var i in units) {
                            if (typeof units[i].afterRouteLoad === 'function') {
                                units[i].afterRouteLoad(req, isReload);
                            }
                        }
                    });
                } else {
                    // not found
                    req.routeDetected = false;
        ...
        back: function (defaultUrl) {
            if (this.hasBack()) {
                return this._nav(this.history[--this.historyIdx], true, true);
            } else if (defaultUrl) {
                return this._nav(defaultUrl, false, true);
            }
    ...
    var View = Route.extend(/** @lends Pilot.View.prototype */{
        ...
        __init: function __() {
            ...
            /*this.on('routestart routeend', this.bound(function (evt, req){
             this.toggleView(evt.type != 'routeend', req);
             }));*/

            this.on('routestart', this.bound(function (evt, req) {
                this.viewIntro(req);
            }));

            this.on('routeend', this.bound(function (evt, req) {
                this.viewOutro(req);
            }));
        ...
        toggleView: function (state, req) {
            var effect = View.toggleEffect[this.toggleEffect];
            this.$el && effect && effect.call(this, this.$el, state, req);
        },

        viewIntro: function (req) {
            this.toggleView(true, req)
        },

        viewOutro: function (req) {
            this.toggleView(false, req)
        },

        beforeRouteLoad: function (req, isReload) {
        },

        afterRouteLoad: function (req, isReload) {
        },
        ...
        render: function () {
            ...
            this.router.trigger('routerender', this.request);
        }
    });
    // View;
