...
class CmfInstallHttpRequestsProfilingCommand extends CmfCommand
{
    ...
    public function handle(): int
    {
        ...
        $this->line(
            '3. Add "' . CmfHttpRequestStatsScaffoldConfig::class . '" to "resources" key in your cmf config (by default: config/peskycmf.php).'
        );
        $this->line(
            'Note: In order to profile all sql queries you may need to place middleware 1st in the list or call PeskyOrmPdoProfiler::init() in AppServiceProvider->register().'
        );
