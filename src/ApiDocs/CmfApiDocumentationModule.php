...
class CmfApiDocumentationModule
{
     ...
     * Note: it is not added automatically to menu items - you need to
     * add it manually to static::menu().
     */
    ...
    /**
     * Provides sections with list of objects of classes that extend
     * CmfApiMethodDocumentation class to be displayed in api docs section.
     * @return array
     * key - section name;
     * value - array that contains names of classes that extend CmfApiDocumentation class.
     */
    ...
    /**
     * Load api docs sections from files in
     * static::api_methods_documentation_classes_folder() and its subfolders.
    ...
    protected function loadClassesFromFileSystem(): array
    {
        ...
        $classFinder = function ($folderPath, array $files) {
            ...
            foreach ($files as $fileName) {
                if (preg_match('%\.php$%i', $fileName)) {
                    ...
                    $parentClassName = class_basename($this->getMethodBaseClass())
                        . '|[a-zA-Z0-9_-]+V1ApiMethodDocumentation|CmfApiDocumentation';
                    if (
                        preg_match(
                            '%^\s*class\s+(\w+)\s+extends\s+(' . $parentClassName . ')%im',
                            $buffer,
                            $classMatches
                        )
                    ) {
                        ...
                        if (
                            preg_match(
                                "%[^w]namespace\s+([\w\\\]+).*?class\s+{$class}\s+%is",
                                $buffer,
                                $nsMatches
                            )
                        ) {
            ...
            // sort classes
            usort($classes, static function ($class1, $class2) {
        ...
        if (!empty($withoutSection)) {
            $translation = (string)$this->getCmfConfig()
                ->transCustom('api_docs.section.no_section');
            $ret[$translation] = $withoutSection;
        }
        foreach ($subFolders as $subFolderName) {
            ...
            if (!empty($classes)) {
                $translation = (string)$this->getCmfConfig()
                    ->transApiDoc('section.' . Str::snake($subFolderName));
                $ret[$translation] = $classes;
            }
        }
        return $ret;
    }
    ...
    {
        return $this->getCmfConfig()->config('api_documentation.folder')
            ?: app_path('Api/Docs');
    }

    public function getMethodBaseClass(): string
    {
        return $this->getCmfConfig()->config('api_documentation.base_class_for_method')
            ?: CmfApiMethodDocumentation::class;
    }

    public function getClassNameSuffix(): string
    {
        return $this->getCmfConfig()
            ->config('api_documentation.class_suffix', 'Documentation');
    }
}
